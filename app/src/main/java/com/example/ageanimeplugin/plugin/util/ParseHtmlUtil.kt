...
object ParseHtmlUtil {
    ...
    fun parseSearchEm(element: Element, imageReferer: String): List<BaseData> {
        ...
        for (i in results.indices){
            var cover = results[i].select("img").attr("src").getImageUrl()
            if (imageReferer.isNotBlank())
                cover = getCoverUrl(cover, imageReferer)
            val title = results[i].select(".cell_imform_name").text()
            val url = results[i].select(".cell_imform_name").attr("href")
...
//        val rankInfoItemDataList = mutableListOf<BaseData>()
        ...
        for (e in ul.indices){
            ...
            for (i in li.indices){
                val rankIdx = li[i].select(".rank_text_idx").text()
